...
# -*- coding: utf-8 -*-
"""
Training script for triplet-based channel charting.

This script implements triplet-based channel charting [1] using channel-state
information (CSI) from the 5G physical uplink shared channel (PUSCH). The method
learns a low-dimensional representation (channel chart) that preserves local
spatial proximity without requiring ground-truth position labels.

Workflow:
1. Load preprocessed CSI dataset (from gen_dataset.py)
2. Extract features and prepare training/test splits
3. Train neural network with triplet loss
4. Evaluate channel chart quality and save results
...
References:
[1] P. Ferrand, A. Decurninge, L. G. Ordonez, and M. Guillaud, "Triplet-based 
    wireless channel charting: Architecture and experiments," IEEE J. Sel. 
    Areas Commun., vol. 39, no. 8, pp. 2361–2373, 2021.

[2] S. Taner, V. Palhares, and C. Studer, "Channel charting in real-world 
    coordinates with distributed MIMO," IEEE Trans. Wireless Commun., 
    vol. 24, no. 9, pp. 7286–7300, 2025.

[3] R. Wiesmayr, F. Zumegen, S. Taner, C. Dick, and C. Studer, "CSI-based user 
...
parser = argparse.ArgumentParser(description='Triplet-based channel charting using CAEZ-5G datasets')
...
runID = '5g_cc_tiplet_only_autocorrelation_robot_measurement_outdoor_2025_10_11_multi_oru_eval_last_500_1'
...
# - 'Triplet': uses timestamp-based triplet loss (and optionally bilateration loss)
...
lambda_b = 0  # For triplet-only training, set to 0
lambda_box = 0  # Weight for bounding box loss
...
n_ref = 0  # Number of ground-truth labeled points for affine transform (0 for triplet-only)
...
num_triplets_per_anchor = 1   #  @TODO adjusts the "true" batch size
...
oru_pos_list = [oru_pos_dict["position_data_oru7_cell41"], oru_pos_dict["position_data_oru8_cell42"], oru_pos_dict["position_data_oru5_cell43"], oru_pos_dict["position_data_oru6_cell51"]] # CAEZ-5G-OUTDOOR
...
# DEBUG:
# A=2 
# bounding_box_epsilon = .15
# bounding_box = np.array([[-1.75-bounding_box_epsilon, -1.5-bounding_box_epsilon],   # lower left corner
#                          [1.25+bounding_box_epsilon, 1.5+bounding_box_epsilon]])
# bounding_boxes_ap = np.repeat([bounding_box], repeats=A, axis=0)
...
###############################################################################
# Compute receive power per AP for bilateration loss (not used when lambda_b=0)
...
if learning_type == 'Supervised': # triplet parameters or loss weights do not matter
    ...
    if lambda_b == 0 and lambda_box == 0:
        ...
        runID = runID + f'-lambda_b{lambda_b}-lambda_box{lambda_box}-powtype{pow_per_ap_type}-Pthresh{P_thresh}-Mp{M_p}-Tc{Tc}-Tf{Tf}-Mt{M_t}-{par_tr.num_triplets_per_anchor}tperanch-bs{batch_size}-lr{lr}-{num_epochs}epochs-hidden{hidden_features[0]}'
